...
const (
	loraNotebookName = "lora.ipynb"
	loraNotebookPath = "resources/" + loraNotebookName
)

// Multi-GPU - Distributed Training with LORA and TrainingHubTrainer
func RunLoraTrainingHubMultiGpuDistributedTraining(t *testing.T) {
	test := support.With(t)
	...
	prefix, _ := support.GetStorageBucketLoraDir()
	if !endpointOK {
		endpoint = ""
	}
	if !bucketOK {
	...
	common.CreateNotebook(test, namespace, userToken, command, cm.Name, loraNotebookName, 0, rwxPvc, common.ContainerSizeMedium)
