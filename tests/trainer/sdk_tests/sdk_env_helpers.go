...
// 3. Neither set: defaults to git main to preserve backward compatibility with legacy notebook behavior
//
// Index behavior:
...
func buildKubeflowInstallExports() string {
	...
	if gitURL != "" {
		...
		// Tweak for backward compatibility: default to git main if nothing is set
		defaultGitURL := "git+https://github.com/opendatahub-io/kubeflow-sdk.git@main"
		exports.WriteString("export KUBEFLOW_INSTALL_FROM_GIT='true'; ")
		exports.WriteString("export KUBEFLOW_GIT_URL=" + shellQuote(defaultGitURL) + "; ")
	}
